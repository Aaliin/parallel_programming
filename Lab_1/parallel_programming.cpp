...
vector<vector<double>> matrix_multiplication(const vector<vector<double>>& matrix1, const vector<vector<double>>& matrix2) {
...
void write_matrix_file(const string& filename, const vector<vector<double>>& matrix, double time) {
    ...
    time_file << "The size: " << rows << " x " << cols << "\tTotal time: " << time << setprecision(5) << " microseconds" << endl;
...
int main() {
    ...
    time(&start); 
    vector<vector<double>> result_matrix = matrix_multiplication(matrix1, matrix2); 
    time(&end);

    write_matrix_file(output_file, result_matrix, double(end - start));
